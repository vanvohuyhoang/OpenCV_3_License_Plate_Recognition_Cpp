...
std::vector<PossiblePlate> detectPlatesInScene(cv::Mat &imgOriginalScene)
{
    ...
    std::cout << std::endl
              << vectorOfPossiblePlates.size() << " possible plates found" << std::endl; // 13 with MCLRNF1 image
    ...
    std::cout << std::endl
              << "plate detection complete, click on any image and press a key to begin char recognition . . ." << std::endl
              << std::endl;
    // cv::waitKey(0);
#endif // SHOW_STEPS
...
#ifdef SHOW_STEPS
    std::cout << std::endl
              << "contours.size() = " << contours.size() << std::endl;                               // 2362 with MCLRNF1 image
